...
import {
    attachAuthCookies,
    attachBackendModeCookie,
} from '@/lib/core/auth/cookies'
...
import { registerWithProvider } from '@/lib/core/services/auth-service'
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const mode = await resolveBackendModeForRequest(
            request,
            body.backendMode,
        )
        ...
        const response = jsonSuccess(result.user, { status: 201 })

        if (result.cookies.forwardedSetCookieHeaders) {
            applyForwardedSetCookieHeaders(
                response,
                result.cookies.forwardedSetCookieHeaders,
            )
        } else if (
