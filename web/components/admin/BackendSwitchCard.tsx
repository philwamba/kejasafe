  ...
  const submit = () => {
    ...
    startTransition(async () => {
      try {
        ...
        const response = await internalApiClient.put<ApiDataEnvelope<BackendSettingDto>>(
        ...
        setMessage(`Active backend switched to ${response.data.data.activeMode}.`);
        router.refresh();
      } catch (switchError) {
        setError(
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-white/5 p-8">
      ...
      <p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-300">
        Switching is persisted in the control-plane settings table and audited in Prisma.
        New internal requests without an explicit backend cookie now resolve against that
        persisted setting.
      </p>

      <div className="mt-6 grid gap-5 md:grid-cols-2">
        <label className="grid gap-2">
          <span className="text-sm font-medium text-white">Active backend</span>
          <select
            value={activeMode}
            onChange={(event) => setActiveMode(event.target.value as BackendMode)}
        ...
        <label className="grid gap-2">
          <span className="text-sm font-medium text-white">Fallback backend</span>
          <select
            value={fallbackMode}
            onChange={(event) => setFallbackMode(event.target.value as BackendMode | "")}
            className="h-12 rounded-2xl border border-white/10 bg-white/5 px-4 text-sm outline-none"
          >
      ...
      <label className="mt-5 grid gap-2">
        <span className="text-sm font-medium text-white">Switch notes</span>
        <textarea
          value={switchNotes}
