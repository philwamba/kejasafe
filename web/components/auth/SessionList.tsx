...
export function SessionList() {
  ...
  if (isLoading) {
    return <p className="text-sm text-stone-300">Loading active sessions...</p>;
  }
      ...
      {sessions.map((session) => (
        ...
        >
          <div className="space-y-1">
            ...
            <p className="text-sm text-stone-300">{session.userAgent ?? "Unknown device"}</p>
          ...
          <button
            ...
            className="h-11 rounded-2xl border border-white/15 px-4 text-sm font-medium text-white transition hover:bg-white/10 disabled:opacity-60"
          >
            {session.isCurrent ? "Sign out this session" : "Revoke session"}
          </button>
        </div>
      ))}
    </div>
