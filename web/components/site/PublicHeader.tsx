    ...
    <header className="sticky top-4 z-30 rounded-2xl border border-stone-200 bg-white/95 shadow-sm backdrop-blur">
      <div className="flex h-16 items-center gap-6 px-5">
        ...
        <nav className="hidden items-center gap-6 text-sm font-medium text-stone-700 lg:flex">
          ...
          {secondaryLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-stone-500 transition-colors hover:text-brand"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="ml-auto flex items-center gap-2">
          <Button asChild variant="ghost" size="lg" className="hidden h-10 rounded-xl px-4 text-sm font-medium text-stone-700 sm:inline-flex">
            <Link href="/login">Sign in</Link>
          </Button>
          <Button asChild size="lg" className="h-10 rounded-xl px-4 text-sm font-semibold">
