...
export function normalizePaginationInput(input: PaginationInput = {}): NormalizedPaginationInput {
  const page = Number.isFinite(input.page) ? Math.max(1, Number(input.page)) : DEFAULT_PAGE;
...
export function emptyPaginatedResult<T>(input: PaginationInput = {}): PaginatedResult<T> {
