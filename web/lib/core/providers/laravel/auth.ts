...
async function statefulPost<T>(
    ...
    const cookieHeader = csrfCookies
        ...
        .join('; ')
    const xsrfCookie = csrfCookies.find(cookie =>
        cookie.startsWith('XSRF-TOKEN='),
    )
    const xsrfToken = xsrfCookie?.split(';')[0].split('=')[1]
    const mergedCookieHeader = [ctx.cookieHeader, cookieHeader]
        .filter(Boolean)
        .join('; ')
    ...
    return {
        ...
        setCookieHeaders: [
            ...csrfCookies,
            ...forwardedCookies(response.headers['set-cookie']),
        ],
    }
}
...
export const laravelAuthProvider: AuthProvider = {
    async me(ctx) {
        try {
            const response =
                await laravelApiClient.get<LaravelAuthPayloadEnvelope>(
                    '/auth/me',
                    {
                        headers: buildHeaders(ctx),
                    },
    ...
    async login(input, ctx): Promise<AuthMutationResult> {
        const response = await statefulPost<LaravelAuthPayloadEnvelope>(
            '/auth/login',
            input,
            ctx,
        )
    ...
    async revokeSession(sessionId: string, ctx: AuthContext): Promise<void> {
        ...
        await laravelApiClient.delete(`/auth/sessions/${sessionId}`, {
            headers: {
                ...buildHeaders(ctx),
                Cookie: [ctx.cookieHeader, cookieHeader]
                    .filter(Boolean)
                    .join('; '),
                ...(xsrfToken
                    ? { 'X-XSRF-TOKEN': decodeURIComponent(xsrfToken) }
    ...
    async requestEmailVerification(ctx: AuthContext): Promise<MessageResult> {
        const response = await statefulPost<{
            message: string
            debugToken?: string
        }>('/auth/verify-email/request', {}, ctx)
