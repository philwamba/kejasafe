...
import type { AuthUserDto, MessageResult, SessionSummaryDto } from "@/lib/shared/types/auth";
...
export async function fetchCsrfToken() {
  const response = await internalApiClient.get<ApiDataEnvelope<CsrfTokenPayload>>(
    "/auth/csrf",
  );
...
export async function fetchCurrentUser() {
  const response = await internalApiClient.get<ApiDataEnvelope<AuthUserDto>>("/auth/me");
...
export async function requestPasswordResetToken(
  ...
  const response = await internalApiClient.post<ApiDataEnvelope<MessageResult>>(
...
export async function fetchSessions() {
  const response = await internalApiClient.get<ApiDataEnvelope<SessionSummaryDto[]>>(
    "/auth/sessions",
  );
...
export async function revokeSession(sessionId: string, csrfToken: string) {
  const response = await internalApiClient.delete<ApiDataEnvelope<MessageResult> | MessageResult>(
