...
import type { PropertyDetailDto, PropertySearchInput } from "@/lib/core/contracts/property";
...
export async function fetchProperties(input: PropertySearchInput = {}) {
  ...
  const response = await internalApiClient.get<ApiListEnvelope<PropertyCardDto>>(path);
...
export async function fetchPropertyBySlug(slug: string) {
  try {
    const response = await internalApiClient.get<ApiDataEnvelope<PropertyDetailDto>>(
      `/properties/${slug}`,
    );
