...
const seedSources = [
  {
    ...
    sourceUrl: "https://www.buyrentkenya.com/listings/3-bedroom-house-for-rent-runda-3931913",
    title: "Furnished 3 Bedroom Garden House in Runda",
    summary: "A quiet furnished home in Runda with a fireplace, mature garden, and staff quarter.",
    description:
    ...
    nearbyPlaces: [
      { name: "Village Market", category: "shopping_centre", distanceKm: 3.4 },
  ...
  },
  {
    ...
    sourceUrl: "https://www.buyrentkenya.com/listings/furnished-4-bedroom-house-for-sale-diani-3931445",
    title: "Resort-Style 4 Bedroom Mansion in Diani",
    summary: "A newly built Diani home with a private pool, balconies, and beach access within minutes.",
    description:
  ...
  },
  {
    ...
    sourceUrl: "https://www.buyrentkenya.com/listings/furnished-3-bedroom-villa-for-sale-kilifi-town-3730646",
    title: "Italian-Style Villa Overlooking Kilifi Creek",
    summary: "A furnished creekside villa with an infinity pool, mature trees, and guest accommodation.",
    description:
    ...
    latitude: -3.6292,
    longitude: 39.8526,
    amenities: ["Swimming Pool", "Garden", "Parking", "Security", "Scenic View"],
    nearbyPlaces: [
      { name: "Kilifi Creek", category: "waterfront", distanceKm: 0.1 },
      { name: "Naivas Kilifi", category: "shopping_centre", distanceKm: 0.3 },
    ],
    featured: true,
  },
  {
    slug: "nanyuki-new-2-bedroom-bungalow",
    sourceUrl: "https://www.buyrentkenya.com/listings/furnished-2-bedroom-house-for-sale-nanyuki-3871345",
    title: "New 2 Bedroom Bungalow in Nanyuki",
    summary: "A newly completed gated bungalow in Nanyuki with en suite bedrooms and strong value pricing.",
    description:
    ...
    nearbyPlaces: [
      ...
      { name: "Nanyuki Teaching and Referral Hospital", category: "hospital", distanceKm: 4.2 },
    ],
    featured: false,
  },
];
...
function getStorageConfig() {
  ...
  if (!bucket || !region || !endpoint || !accessKeyId || !secretAccessKey || !publicBaseUrl) {
    return null;
  }
...
function extractImageUrls(html, limit = 3) {
  ...
  const candidates = normalized.match(/https:\/\/i\.roamcdn\.net\/[^"'\\s<]+/g) ?? [];
...
async function fetchSourceImages(sourceUrl) {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch source listing ${sourceUrl} (${response.status})`);
  }
...
async function upsertLocationModels() {
  ...
  const uniqueAmenities = [...new Set(seedSources.flatMap((entry) => entry.amenities))];
...
async function seedProperties() {
  ...
  for (const entry of seedSources) {
    ...
    const neighborhood = maps.neighborhoodMap.get(`${entry.county}:${entry.neighborhood}`);
    ...
    for (const [index, imageUrl] of imageUrls.entries()) {
      ...
      const mirrored = await mirrorImage(imageUrl, storageKey, storageConfig, s3Client);
      mirroredImages.push({
        ...mirrored,
    ...
    const property = await prisma.property.upsert({
      ...
      update: {
        ...
        hasSecurityFeatures: entry.amenities.includes("Security") || entry.amenities.includes("CCTV"),
        hasInternetAvailability: entry.amenities.includes("Fibre Internet"),
    ...
    await prisma.propertyNearbyPlace.deleteMany({ where: { propertyId: property.id } });
